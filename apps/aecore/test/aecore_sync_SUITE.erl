...
mine_one_block(N) ->
    ...
    ResumeRes = rpc_call(N, aec_miner, resume, []),
    ct:log("aec_miner:resume() (~p) -> ~p", [N, ResumeRes]),
    receive
    ...
    after 30000 ->
            ...
            SuspendRes = rpc_call(N, aec_miner, suspend, []),
            ct:log("SuspendRes() (~p) -> ~p", [N, SuspendRes]),
        ...
        {From, Ref, {subscribe, Event}} ->
            case lists:keymember(Event, 2, Subs) of
                ...
                false ->
                    ...
                    error_logger:info_report([{?MODULE, proxy_subscribe},
                                              {event, Event},
                                              {result, Res}]),
                    From ! {Ref, Res},
                    proxy_loop([{From, Event}|Subs], Events)
...
set_subscriptions() ->
    [aec_events:subscribe(E) || E <- [block_created,
                                      chain_sync]],
    ok.
...
make_multi(Config) ->
    [setup_node(N, Config) || N <- [dev1, dev2, dev3]].
